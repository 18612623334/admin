                ...
                <div class="ibox-content">
                    <div method="get" class="form-horizontal">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="password" name="password" placeholder="密码（如不填择不修改密码,最多输入15字符）" class="form-control">
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="password" name="password_confirm" placeholder="密码（如不填择不修改密码,最多输入15字符）" class="form-control">
                            </div>
                        </div>
                        @if($admin_data['id'] != '1')
                            <div class="hr-line-dashed"></div>
                                ...
                                <div class="col-md-8">
                                    <select class="form-control" name="group_id">
                                        ...
                                        @foreach($group_list as $k=>$v)
                                            <option value="{{$v['gid']}}" @if($v['gid'] == $admin_data['group_id']) selected @endif>{{$v['group_name']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
<script>
    ...
    $('.btn-primary').on('click',function(){
        ...
        postMsg.password = $('input[name=password]').val();
        postMsg.password_confirm = $('input[name=password_confirm]').val();
        ...
        $.ajax({
            type: "POST",
            url: "{{ URL::route('admin.update-data')}}",
            headers: {
